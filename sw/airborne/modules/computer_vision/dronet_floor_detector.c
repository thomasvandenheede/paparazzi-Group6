...
struct color_object_t global_filter;
...
static struct image_t *object_detector(struct image_t *img, uint8_t camera_id __attribute__((unused)))
{
  ...
  global_filter.color_count = count;
  global_filter.x_c = x_c;
  global_filter.y_c = y_c;
  global_filter.updated = true;
...
void dronet_floor_detector_init(void)
{
  memset(&global_filter, 0, sizeof(struct color_object_t));
  pthread_mutex_init(&mutex, NULL);
    ...
    cv_add_to_device(&COLOR_OBJECT_DETECTOR_CAMERA, object_detector, COLOR_OBJECT_DETECTOR_FPS, 1);
  #endif
}
...
void dronet_floor_detector_periodic(void)
{
  printf("[floor_detector] periodic function called\n");

  static struct color_object_t local_filter;
  pthread_mutex_lock(&mutex);
  memcpy(&local_filter, &global_filter, sizeof(struct color_object_t));
  pthread_mutex_unlock(&mutex);

  if(local_filter.updated){
    AbiSendMsgVISUAL_DETECTION(COLOR_OBJECT_DETECTION2_ID, local_filter.x_c, local_filter.y_c,
        0, 0, local_filter.color_count);
    local_filter.updated = false;
  }
}
